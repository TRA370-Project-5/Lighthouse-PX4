 ...
 */
static void augmentFramesInWorkspace(pulseProcessorV2PulseWorkspace_t* pulseWorkspace) {
...
static void clearWorkspace(pulseProcessorV2PulseWorkspace_t* pulseWorkspace) {
...
static void calculateAngles(const pulseProcessorV2SweepBlock_t* latestBlock, const pulseProcessorV2SweepBlock_t* previousBlock, pulseProcessorResult_t* angles) {
    ...
    angles->lastUsecTimestamp[channel] = hrt_absolute_time();
}

static bool isBlockPairGood(const pulseProcessorV2SweepBlock_t* latest, const pulseProcessorV2SweepBlock_t* storage) {
...
static bool handleCalibrationData(pulseProcessor_t *state, const pulseProcessorFrame_t* frameData) {
    bool isFullMessage = false;

    if (frameData->channelFound && frameData->channel < DECK_LIGHTHOUSE_MAX_N_BS) {
...
bool handleAngles(pulseProcessor_t *state, const pulseProcessorFrame_t* frameData, pulseProcessorResult_t* angles, int *baseStation, int *axis) {
    ...
    for (int i = 0; i < nrOfBlocks; i++) {
        ...
        if (channel < DECK_LIGHTHOUSE_MAX_N_BS) {
