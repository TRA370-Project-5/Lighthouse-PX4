...
static int findSyncTime(const pulseProcessorPulse_t pulseHistory[], uint32_t *sync0Time);
static bool getSystemSyncTime(const uint32_t syncTimes[], size_t nSyncTimes, uint32_t *syncTime);
...
static bool isSync(pulseProcessorV1_t *stateV1, unsigned int timestamp)
{
 ...
 */
static int getBaseStationId(pulseProcessorV1_t *stateV1, unsigned int timestamp) {
  int baseStation = 0;
...
static bool isNewSync(uint32_t timestamp, uint32_t lastSync) {
...
static bool processSync(pulseProcessor_t *state, unsigned int timestamp, unsigned int width, pulseProcessorResult_t* angles, int *baseStation, int *axis, bool* calibDataIsDecoded) {
  ...
  if (isNewSync(timestamp, stateV1->lastSync)) {
    if (isSync(stateV1, timestamp)) {
      ...
      int baseStation1 = getBaseStationId(stateV1, timestamp);

      if (baseStation1 == 1 && stateV1->baseStationsSynchronizedCount < 2) {
        resetSynchronization(stateV1);
      } else {
        storeSyncData(stateV1, baseStation1, timestamp, width);
      }
    } else {
...
uint8_t pulseProcessorV1AnglesQuality() {
  return __builtin_popcount(validAngles)*1.0/(PULSE_PROCESSOR_N_SWEEPS*PULSE_PROCESSOR_N_SENSORS*DECK_LIGHTHOUSE_MAX_N_BS)*255;
}
